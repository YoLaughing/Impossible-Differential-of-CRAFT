...
if __name__ == "__main__":
    ...
    for i in range(0, parameters["blocksize"]):
        t1 = [0 for ti in range(0, parameters["blocksize"])]
        for val in range(1, 16):
            t1[i] = val
            e1 = [0 for ei in range(0, parameters["blocksize"])]
            search_space.append(copy.deepcopy([t1, e1]))
    ...
    while distinguish_find:
        ...
        while search_count < len(search_space):
            parameters["key0"] = copy.deepcopy(search_space[search_count][1])
            parameters["key1"] = copy.deepcopy(search_space[search_count][1])
            parameters["tweak"] = copy.deepcopy(search_space[search_count][0])
            parameters["indiff"] = copy.deepcopy(search_space[search_count][1])
            ...
            if flag:
                ...
                rf.write("tw = {}\n".format(stp.listTostring(parameters["tweak"], parameters["sbox_size"])))
                rf.close()
                distinguish_find = True
                break
            else:
    ...
    while search_count < len(search_space):
        ...
        parameters["indiff"] = copy.deepcopy(search_space[search_count][1])
        parameters["outdiff"] = copy.deepcopy(search_space[search_count][1])
        parameters["max_solve_file"] = folder + "////" + parameters["cipher_name"] + "_round{}_{}_max.stp".format(
            begin_round, begin_round + max_distinguish_rounds)
